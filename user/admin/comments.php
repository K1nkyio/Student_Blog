...
// list comments
$res = $conn->query("SELECT c.*, p.title AS post_title FROM comments c LEFT JOIN posts p ON c.post_id = p.id ORDER BY c.created_at DESC");
...
.comments-tab:hover { color: var(--ink-mid); }
.comments-tab.active {
  color: var(--ink);
...
.comments-tab.active .comments-tab-count {
  background: var(--ink);
  color: #fff;
}
...
.comment-row[data-status="approved"]::before { background: var(--green); }
.comment-row[data-status="pending"]::before  { background: var(--amber); }
...
.comment-avatar--approved { background: var(--green-dim); color: var(--green); }
.comment-avatar--pending  { background: var(--amber-dim); color: var(--amber); }
...
.comments-empty p  { font-size: .875rem; }
...
@media (max-width: 420px) {
  .comments-toolbar-inner { flex-direction: column; align-items: stretch; }
}
</style>
...
<div class="comments-page">
  ...
  <div class="comments-hero">
    <div class="hero-inner">
      ...
      <div class="hero-stats">
        <div class="hero-stat">
          <span class="hero-stat-num"><?php echo $total; ?></span>
          <span class="hero-stat-label">Total</span>
        </div>
        <div class="hero-stat">
          <span class="hero-stat-num"><?php echo $approved; ?></span>
          <span class="hero-stat-label">Approved</span>
        </div>
        <div class="hero-stat">
          <span class="hero-stat-num"><?php echo $pending; ?></span>
  ...
  <div class="comments-body">
    ...
    <!-- Tabs -->
    <div class="comments-tabs" id="commentTabs">
      <button class="comments-tab active" data-filter="all">
        All <span class="comments-tab-count" id="countAll"><?php echo $total; ?></span>
      </button>
      <button class="comments-tab" data-filter="pending">
        Pending <span class="comments-tab-count" id="countPending"><?php echo $pending; ?></span>
      </button>
      <button class="comments-tab" data-filter="approved">
        Approved <span class="comments-tab-count" id="countApproved"><?php echo $approved; ?></span>
      </button>
    </div>

    <!-- Results bar -->
    <div class="results-bar">
      <span><strong id="resultsCount">0</strong> comments found</span>
    ...
    <?php if (!empty($all_comments)): ?>
      ...
      <div class="comments-list" id="commentsList">
        <?php foreach ($all_comments as $i => $r):
          $status     = $r['approved'] ? 'approved' : 'pending';
          $initial    = strtoupper(substr(trim($r['name'] ?? 'A'), 0, 1)) ?: '?';
          $post_title = $r['post_title'] ?? 'Unknown Post';
          $date_str   = !empty($r['created_at']) ? date('M d, Y · H:i', strtotime($r['created_at'])) : '';
        ?>
        <div class="comment-row"
          data-status="<?php echo htmlspecialchars($status); ?>"
          data-name="<?php echo htmlspecialchars(strtolower($r['name'] ?? '')); ?>"
          data-post="<?php echo htmlspecialchars(strtolower($post_title)); ?>"
          data-comment="<?php echo htmlspecialchars(strtolower($r['comment'] ?? '')); ?>"
          style="animation-delay: <?php echo $i * 35; ?>ms">

          <!-- avatar -->
          <div class="comment-avatar comment-avatar--<?php echo htmlspecialchars($status); ?>">
          ...
          <div class="row-body">
            <div class="row-badges">
              <span class="badge badge--<?php echo htmlspecialchars($status); ?>">
                <?php echo $status === 'approved' ? '✓ Approved' : '⏳ Pending'; ?>
              </span>
            </div>

            <h3 class="row-post-title"><?php echo htmlspecialchars($post_title); ?></h3>
            ...
            <div class="row-meta">
              <span class="row-meta-item">
                ...
                <?php echo htmlspecialchars($r['name'] ?? 'Anonymous'); ?>
              </span>
              <?php if ($date_str): ?>
              <span class="row-meta-item">
                <!-- calendar icon -->
                ...
                <?php echo htmlspecialchars($date_str); ?>
              </span>
              <?php endif; ?>
            </div>

            <p class="row-comment-text"><?php echo nl2br(htmlspecialchars($r['comment'] ?? '')); ?></p>
          </div>

          <!-- actions -->
          <div class="row-actions">
            <?php if (!$r['approved']): ?>
              <a class="btn btn-approve" href="?approve=<?php echo (int)$r['id']; ?>">
            ...
            <button class="btn btn-delete"
              data-delete-id="<?php echo (int)$r['id']; ?>"
              data-delete-name="<?php echo htmlspecialchars($r['name'] ?? 'this comment'); ?>"
              type="button">
              <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <?php else: ?>
      <div class="comments-empty">
        ...
        <p>Comments submitted by readers will appear here for moderation.</p>
      </div>
    <?php endif; ?>
...
(function () {
  ...
  const tabs          = document.querySelectorAll('.comments-tab');
  ...
  const rows     = list ? Array.from(list.querySelectorAll('.comment-row')) : [];
  const pageSize = 10;
  let currentPage    = 1;
  let activeFilter   = 'all';

  /* ── Match ── */
  const matches = (row, query, filter) => {
    const name    = row.dataset.name    || '';
    const post    = row.dataset.post    || '';
    const comment = row.dataset.comment || '';
    const status  = row.dataset.status  || '';

    return (!query  || name.includes(query) || post.includes(query) || comment.includes(query))
  ...
  const render = () => {
    ...
    const filtered = rows.filter(r => matches(r, query, activeFilter));
  ...
  document.querySelectorAll('[data-delete-id]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (modalConfirm) modalConfirm.href         = `?delete=${id}`;
