...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        ...
        if (!$error) {
            $stmt = $conn->prepare("INSERT INTO posts (title, excerpt, content, author_id, image, category_id, visible, review_status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')");
            $stmt->bind_param("sssissi", $title, $excerpt, $content, $user_id, $imagePath, $category_id, $visible);
            if ($stmt->execute()) {
...
/* inputs — same as admin-header .form-control */
...
/* submit — .btn-primary from admin-header */
.pf-btn-submit {
  display: inline-flex;
  align-items: center;
  gap: .45rem;
...
<div class="pf-heading">
  <div>
    ...
    <div class="pf-eyebrow">New Post</div>
    <h1 class="pf-title">Create<em> Post</em></h1>
    <p class="pf-sub">Draft content here and send it to the super admin review queue before it appears in Explore.</p>
  </div>
</div>
    ...
    <form method="post" enctype="multipart/form-data" id="postForm">
      ...
      <div class="pf-row-2">
        <div class="pf-field">
          <label class="pf-label" for="title">
            Title
            <span class="pf-label-hint">A clear, descriptive headline for your post</span>
          </label>
      ...
      <div class="pf-field">
        <label class="pf-label" for="excerpt">
          Excerpt
          <span class="pf-label-hint">A brief summary shown in post previews (150–300 characters)</span>
        </label>
        <textarea id="excerpt" name="excerpt" class="pf-textarea" style="min-height:100px;"
                  placeholder="Write a compelling summary…" maxlength="500"></textarea>
      </div>
      ...
      <div class="pf-field">
        <label class="pf-label" for="content">
          Content
          <span class="pf-label-hint">Write your full post content</span>
        </label>
      ...
      <div class="pf-field">
        <label class="pf-label">
          Featured Image
          <span class="pf-label-hint">Optional — upload a cover image for your post</span>
        </label>
      ...
      <div class="pf-field">
        ...
        <label class="pf-toggle-wrap" for="visible">
          <div class="pf-toggle-info">
            <div class="pf-toggle-title">Visible After Approval</div>
            <div class="pf-toggle-desc">If approved, this post will be shown publicly. Turn this off to keep it hidden even after review.</div>
          </div>
          <input type="checkbox" id="visible" name="visible" class="pf-toggle-input" checked>
...
(function () {
  ...
  document.getElementById('postForm')?.addEventListener('submit', function () {
    ...
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = '<svg style="animation:spin .7s linear infinite" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 12a9 9 0 1 1-9-9"/></svg> Submitting…';
    }
  });
